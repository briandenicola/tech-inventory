...
| T08 | Integration tests for JWT validation | Apone | Add integration tests: valid JWT → 200 OK; expired JWT → 401; invalid signature → 401; missing `app_roles` → 401; empty `app_roles` → 401; `admin` role can access Admin-only endpoints; `viewer` role blocked from write endpoints. | 8+ integration tests green; JWT token factory helper reusable. | T06, T07 | Spec §6.1 |
...
### Round Breakdown:
- **Round 0 (Foundation)**: 5 tasks (T01-T05) — [Brian] + [P] parallel
