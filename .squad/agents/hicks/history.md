...
**2026-05-18 (Phase 1 Round 6):** T32–T38, T40, and T41 landed in `src/TechInventory.Api` with classic attribute-routed controllers, a shared `Common/ControllerResultExtensions.cs` mapper, global `IExceptionHandler` ProblemDetails conversion, and Development-only auth bypass guarded by `Auth:DevBypass`. OpenAPI now serves at `/openapi/v1.json`, controllers are `[Authorize]` by default, and audit stamping reads `oid`/`sub` via `Authentication/HttpContextCurrentUserService.cs`. Smoke-tested on `http://localhost:8080`: `GET /openapi/v1.json` → 200 JSON, `GET /api/v1/devices` → `{"items":[],"totalCount":0,"page":1,"pageSize":25}`, `POST /api/v1/brands` with `{"name":"TestBrand2"}` → 201 Created (`Location: http://localhost:8080/api/v1/brands/5c5a57c9-3264-4444-98ed-433c0ba9c8d9`), `POST /api/v1/brands` with empty name → 400 Validation ProblemDetails, and `GET /api/v1/brands/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa` → 404 ProblemDetails. Verification: `dotnet format --verify-no-changes` ✅, `dotnet build src\TechInventory.Api\TechInventory.Api.csproj -c Release` ✅, repo-root `dotnet build -c Release` / `dotnet test -c Release` blocked by Apone's in-flight `tests\TechInventory.IntegrationTests\Controllers\DevAuthBypassTests.cs(94,40)` compile error.
