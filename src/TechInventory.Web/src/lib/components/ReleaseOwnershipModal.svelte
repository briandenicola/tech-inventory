<!--
	ReleaseOwnershipModal.svelte — Ownership release confirmation modal (T25)
	...
	Related: specs/002-frontend-mvp/spec.md J9 (T25), Constitution §4.2, §4.3
-->
<script lang="ts">
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			console.error('[ReleaseOwnershipModal] Submit failed:', err);
		} finally {
			isSubmitting = false;
		}
	}
...
>
	...
	>
		...
		<div class="border-b border-neutral-200 px-6 py-4 dark:border-neutral-800">
			<h2 id="release-modal-title" class="text-lg font-semibold text-neutral-900 dark:text-neutral-100">
				{t('devices.release.modal.title')}
			</h2>
		</div>
		...
		<form onsubmit={handleSubmit} class="space-y-4 px-6 py-4">
			...
			<div class="flex gap-3 rounded-lg bg-warning-50 p-3 dark:bg-warning-950">
				...
				<div class="text-sm text-warning-900 dark:text-warning-100">
					...
					<p class="mt-1">{t('devices.release.modal.body')}</p>
				</div>
			</div>
			...
			<div class="flex justify-end gap-3 border-t border-neutral-200 pt-4 dark:border-neutral-800">
				...
				<button
					type="submit"
					disabled={isSubmitting}
					class="inline-flex items-center gap-2 rounded-lg bg-warning-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-warning-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-warning-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-warning-500 dark:hover:bg-warning-600"
				>
					{#if isSubmitting}
						<svg
					...
					{:else}
						{t('devices.release.modal.confirm')}
					{/if}
				</button>
			</div>
		</form>
