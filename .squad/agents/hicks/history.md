...
**2026-05-18 (Phase 1 Round 5):** Application handlers T20–T28 complete. Added concrete command/query records, handlers, validators, and DTOs under `src/TechInventory.Application/` for Devices, Brands, Categories, Owners, Locations, Networks, and Tags; shipped `AddTagToDeviceCommand`, `RemoveTagFromDeviceCommand`, `ClaimDeviceOwnershipCommand`, and shared `PagedResponse<T>`. `IAuditContext` pattern is now crystallized: creates only set entity/action metadata, while update/delete/ownership/tag-removal handlers stash BEFORE snapshots from response DTOs or focused audit records and let `AuditBehavior` serialize AFTER from the request. Category handlers established the subtree pattern (root-level pagination, descendant depth rebalance on move, cascade deactivate on delete), and owner delete now explicitly guards the active-owner invariant by refusing archival while any device still points at that owner. Verified on Windows with `dotnet format --verify-no-changes`, `dotnet build -c Release`, and `dotnet test -c Release` (workspace summary: 182 succeeded, 78 skipped).
