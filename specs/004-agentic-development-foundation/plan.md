...
| T102 | T103 | The matrix is the work list of what must be rebuilt, and where. **`DONE`** — `coverage-migration.md` §12 (5 `H-` items done, 20/22 `C-` items done + accepted gaps C-18/C-20/G-10, 16 `M-` items published, 10 `G-` gaps owned). |
...
- **Scope of the exception:** the 16-check browser/PWA checklist at
  [`docs/testing/manual-pwa-validation.md`](../../docs/testing/manual-pwa-validation.md)
  (`M-01`…`M-16`, published by T102 and corrected by QC, `coverage-migration.md` §12) is **run by
...
- **Class:** `REVIEWED`, not `ENFORCED` — never reported as automated
  coverage, and no `task verify` stage or CI job may claim these sixteen
  checks as satisfied by a green run.

### 6.4 Performance budgets reviewed manually — recorded under §2.10 (GitHub Copilot CLI, 2026-09-03)

**Named exception, scoped, attributed, and dated — not a silent gap.**

- **Rule changed:** constitution §6.5.9/§9 and PRD §7.5.2 previously mandated
  Lighthouse CI, while ADR 0002 prohibited automated browser tooling in every
  role. No Lighthouse harness, Task target, or workflow existed.
- **Decision:** `briandenicola` selected "align with ADR" during the
  post-major-work QC audit on 2026-09-03. The nonexistent mandatory CI gate is
  removed; the performance budgets remain required through manual release
  check P-01 in `docs/testing/manual-pwa-validation.md`.
- **Owner:** `briandenicola`.
- **Scope:** route performance budgets only. Accessibility remains automated
  with axe-core component tests; security and functional gates are unchanged.
- **Class:** `REVIEWED`, not `ENFORCED`. A green `task verify` run supplies no
  performance evidence.
- **Review / closure trigger:** review P-01 at every release. Replacing it with
  automation requires a new ADR superseding ADR 0002, with an implemented,
  fail-closed gate and an owner.
