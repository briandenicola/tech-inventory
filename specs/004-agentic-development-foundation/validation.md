...
document: validation
status: T001–T105 DONE and reviewer-approved; post-major-work QC blockers remediated; full verification and merge readiness remain open
last_updated: 2026-09-03 (post-major-work QC corrections applied; performance policy aligned with ADR 0002 by human decision)
...
**No gate in this work package is merge-required on GitHub.** T105 enumerated
the intended contexts and produced the branch-protection recommendation, but
the setting remains unapplied pending observation of a real PR run. Local
fail-closed evidence is not represented as GitHub enforcement.
...
| **AC-006** | Valuable coverage live at lower layers — real HTTP integration/contract, component tests, and an owned manual checklist for browser-only behaviour; seed-fixture drift replaced by typed builders/generated-contract checks, `fixtures/api.ts` **not** repaired for continued use | T102 | ✅ `DONE` — **independently verified, APPROVED** (§6), with post-major-work QC correction | `coverage-migration.md` §12 (5 `H-` done; 20/22 `C-` done; C-18 partial with `/devices` as **G-09**; the six reference-admin route compositions omitted from T103's C-18 set are explicit **G-10**; C-20 accepted gap; 16-check manual checklist published). The H-05 import regression now owns a synthetic fixture instead of depending on gitignored household data. | n/a — manual checklist is `REVIEWED`, not a gate; HTTP/component suites supply automated evidence. |
| **AC-007** | Coverage migration matrix and deletion map covering **every** existing Playwright spec — test count, behaviour, destination layer, named replacement or accepted-gap owner; no spec deleted without a row; **precedes** T101/T102 deletion | T103 | ✅ `DONE`, with post-major-work QC correction | `coverage-migration.md` §2–§10; the QC pass corrected the accessibility-smoke disposition omitted by the original matrix by adding **G-10** and **M-16**. | n/a — analysis task, no gate |
...
| 2026-09-02 | AC-006 | T102 | `coverage-migration.md` §12 — 5 `H-` backend tests done; 20/22 `C-` frontend items done; C-18 partial with `/devices` as G-09; C-20 accepted. Post-major-work QC added G-10/M-16 for six reference-admin route compositions omitted by the original migration matrix and replaced H-05's workstation-only data dependency with a test-owned fixture. | EVIDENCE RECORDED, CORRECTED, INDEPENDENTLY VERIFIED, then QC-CORRECTED 2026-09-03 | Apone; Bishop; Hicks; Ripley; GitHub Copilot CLI |
| 2026-09-02 | AC-007 | T103 | `coverage-migration.md` §1–§11 — 17 specs and 11 support files given a disposition; post-major-work QC corrected the omitted reference-admin accessibility disposition, yielding 16 browser/PWA checks and 10 accepted gaps. | EVIDENCE RECORDED; QC-CORRECTED 2026-09-03 | Apone; GitHub Copilot CLI |
...
| **Doc alignment** — `brief.md`, `plan.md`, `tasks.md`, `validation.md`, `coverage-migration.md` must agree without overclaiming route-level axe or browser behaviour | **CLOSED after QC correction** | Current sections record C-18 as 5 of its 6 planned route harnesses with `/devices` as G-09, the six omitted reference-admin route compositions as G-10/M-16, and performance review as P-01. None is reported as automated coverage. |
