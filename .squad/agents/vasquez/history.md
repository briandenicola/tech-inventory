...
**2026-05-21 (FAB containing-block regression fix #2):**
The PullToRefresh content wrapper (`data-testid="pull-to-refresh-content"`) in `src/lib/components/PullToRefresh.svelte` had `transition-transform duration-200 ease-out` applied UNCONDITIONALLY as static classes. While commit 39eb0c5 correctly made `will-change-transform` and inline `transform` conditional on `isActive`, the `transition-property: transform` from the `transition-transform` class remained always-on. In WebKit (iOS Safari / PWA standalone), `transition-property: transform` on an element creates a containing block for `position: fixed` descendants even when no active transform exists (WebKit bug 160953). This caused AddDeviceFab and BackToTopFab to anchor to the PullToRefresh wrapper instead of the viewport — appearing mid-page over device cards.

**Fix:** Made `transition-transform`, `duration-200`, and `ease-out` all conditional on `isActive` via Svelte `class:` directives, matching the pattern already used for `will-change-transform`. At rest, the content div now has ZERO transform-related CSS properties, so fixed descendants resolve against the viewport. Trade-off: pull snap-back is now instant (no 200ms ease) when gesture is cancelled; acceptable UX.

**Root cause chain:** PullToRefresh.svelte content div → `transition-property: transform, translate, scale, rotate` (generated by Tailwind v4.3.0's `transition-transform` utility) → WebKit treats as containing block → FABs fixed-position resolves against wrapper not viewport.
