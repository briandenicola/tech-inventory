...
## 2026-05-19 (evening) — PWA field-test backlog (F026–F033) + phase-model rewrite

Two artifacts shipped this session, both doc-only (no code changes):

1. **PWA field-test backlog batch.** Brian ran the production PWA through a hands-on testing session and surfaced 18 distinct issues. Captured as eight new triaged backlog entries (`specs/_backlog/F026..F033`) following the `_TEMPLATE.md` shape, with primary owner, support cast, acceptance criteria, dependencies, and history each citing the originating session item number. Updated `specs/_backlog/README.md` index table + roll-up (12 → 20 total backlog files; F026, F030 marked P1 as the most-blocking). Session plan with the raw-feedback → backlog mapping and squad routing persisted at `~/.copilot/session-state/85fa4627-…/plan.md`. F032 doubles as the answer to Brian's question "what happened to the reporting backlog?" — Phase-4 reporting had lived only in PRD/plan prose; F032 is the first concrete `F0XX-reporting-*.md`.

2. **Phase model rewrite (PRD §13).** Brian called for a full phase rework after noticing drift between `docs/prd.md` §13 (P0–P5 as planned) and `specs/002-frontend-mvp/spec.md:4` (Phase 2 as shipped). Rewrote PRD §13 to match what actually shipped: **P0 Foundation → P1 Core API → P2 Frontend MVP+Auth → P3 Production Launch → P4 Continuous Iteration**. Old P2 (Auth) and old P4 (Web PWA) collapsed into canonical P2 (they shipped together as one slice). Old P3 (Infrastructure) rolled into where the work actually landed (P0 + P2). Old P5 (Hardening & Launch) became P3 — production cutover happened inside the frontend MVP effort, validated 2026-05-19 at `inventory.denicolafamily.com`. **P4 is "no further linear phases; work pulls from `specs/_backlog/`."**

   Live docs rewritten: `docs/prd.md` §13 (full new section + historical footnote), `.copilot-state.md` (Phase line + Next action), `.squad/identity/now.md` (was wildly stale "team just hired" → now reflects P4 + F026 recommendation), `specs/_backlog/README.md` (Phase-4 wording note), `specs/_backlog/F032-*.md` (re-framed Phase-4-Reporting reference), `specs/_backlog/F021-*.md` (one Phase-2 reference softened).

   Historical/spec docs got **one-line preambles** so internals stay correct (they describe what *did* ship): `specs/001-core-api/{spec,plan,tasks}.md`, `specs/002-frontend-mvp/{spec,plan,tasks}.md`, `docs/auth-design.md`, `docs/threat-model.md`, `.github/workflows/README.md`, `.github/T47-CI-SETUP-CHECKLIST.md`. The preambles point at the new phase model and explain that internal "Phase N" labels are pre-rewrite history. Auth-design.md preamble also corrects the dangling `specs/002-auth-entra` reference (never created — auth shipped inside `specs/002-frontend-mvp/`).

   Explicitly **left untouched** as chronological record: all `.squad/log/*`, `.squad/orchestration-log/*`, `.squad/decisions.md` + `decisions/merged/*`, `.squad/sessions/*`, `.squad/agents/*/history.md`, spec History sections at end of spec/plan/tasks files. Those describe what happened on what date and rewriting them would falsify history.

   Constitution has zero "phase" references → untouched. `.github/agents/squad.agent.md` "Phase 1" reference is about the Coordinator's *init mode*, not project phases → untouched.

**Recommended next action**: F026 (PWA quick-win UX pack — transparent-overlay bug, `+` FAB, Active-default filter, 2-up cards, audit modal, pinch-zoom off). Sequencing for the field-test batch documented in the session plan.
