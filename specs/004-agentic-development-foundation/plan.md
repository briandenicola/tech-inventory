...
§7.8, `coverage-migration.md` §13.10). **T104 is `VALIDATING`**: it was
...
closing all three blockers (`validation.md` §11) — `task verify` now runs
end-to-end and exits 0. **Not self-approved; awaiting Apone re-review.**
Hudson remains locked out for this cycle; **T105 is not authorized to
begin** until that re-review passes. T105 remains
`APPROVED` and **not started**. T102 touched test files across
...
### T101–T105 — implementation · T103, T102, **T101 `DONE`** (T101 rejected → revised by Apone → reviewer-APPROVED, `validation.md` §7.8); **T104 `VALIDATING`** (rejected by Apone → revised by Hicks 2026-09-02, `validation.md` §11, awaiting re-review); T105 `APPROVED`, not started
...
| T104 | T101, T102 | The verification surface must reflect the retired harness and the migrated suites. **T101 and T102 are both `DONE` and reviewer-approved. T104 is `VALIDATING`**: rejected at Apone's independent reviewer gate 2026-09-02 (`validation.md` §10) on B-1/B-2/B-3, then **revised by Hicks 2026-09-02** closing all three (`validation.md` §11) — both §7.8.5/§7.8.6 conditions remain honored (contradiction not deepened; F-4 closed; `docker-compose.e2e.yml`/`.env.e2e` deleted with no real non-browser role). **Not self-approved; awaiting Apone re-review.** Hudson remains locked out for this cycle. |
| T105 | T104 | Required check names must exist before they can be required or tampered with. **Not authorized to begin** — `check:client-drift` and `check:vulnerable` changed shape under the T104 revision (`validation.md` §11.1, §11.2) and T104 has not yet cleared Apone's re-review. |
