...
**2026-05-18 (Phase 1 Round 5):** Domain coverage fully recovered and handler tests bound to Hicks's landed T20–T28 handlers. Added targeted Domain tests for `AuditEvent` default-timestamp guards, `ImportBatch` EF-safe/private-constructor + UTC timestamp behavior, and `Currency.ToString()`, plus removed a dead duplicate child-depth guard in `Category`; fresh merged coverage is now **Domain 100.00%**, **Application 85.89%**, **Infrastructure 88.98%**. Started with 102 skip-when-waiting handler scaffolds while Hicks was still landing code, then converted every T20–T28 scaffold into executable xUnit/NSubstitute tests once the handlers appeared; final backend suite is **266 passed / 0 skipped**. Encoded handler-contract assumptions include: create/update mutations fail with `Error.Code = "Conflict"` on inactive references or duplicate-name collisions, update/delete/ownership/tag-removal stash BEFORE snapshots in `IAuditContext`, category list paginates root nodes while keeping descendants nested, and owner delete is blocked while devices still reference that owner.
