...
**2026-05-18 (Phase 1 Round 7):** T29/T30/T31/T39/T42/T48 landed in the backend close-out pass. Added CSV import preview/commit handlers plus `DeviceImportProcessingService`, `ImportContracts`, and `ImportsController`; commit re-parses the upload, creates missing Brand/Category/Owner/Location records, persists immutable `ImportBatch` rows, and emits one audit event per created device through the expanded multi-entry `IAuditContext`. Added export support through `ExportDevicesQuery`, `IDeviceExportService`, `DeviceRepository` projections, and `ExportsController` with async CSV/JSON response writing. `Program.cs` now seeds a default `Primary Household` (`USD`) when the database is empty and supports `export-openapi`; `Taskfile.yml` now exposes `openapi:export`, and repo-root `openapi.yaml` is generated from runtime. Smoke tests on `http://localhost:8080` passed for imports preview/commit/list, exports JSON/CSV, and `/openapi/v1.json`. Verification: repo-root `dotnet format --verify-no-changes` ✅, `dotnet build -c Release` ✅, `dotnet test -c Release` ✅ (**370 total / 369 succeeded / 1 skipped / 0 failed**), and `dotnet run --project src\TechInventory.Api\TechInventory.Api.csproj -c Release --no-build -- export-openapi` ✅.
