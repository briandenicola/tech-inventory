<!--
	ClaimOwnershipModal.svelte — Ownership claim confirmation modal (T24)
	
	Simpler than DeleteDeviceModal: no type-to-confirm (less destructive).
	Yes/Cancel confirmation with current owner display.
	Focus trap (roll-your-own, same pattern as DeleteDeviceModal per D-071).
	Backdrop click DOES close (less destructive).
	
	Props:
	- deviceName: string (for display)
	- currentOwnerName: string | null (display current owner or "unowned")
	- onConfirm: () => Promise<void>
	- onCancel: () => void
	
	Related: specs/002-frontend-mvp/spec.md J9 (T24), Constitution §4.2, §4.3
-->
<script lang="ts">
	...
	let { deviceName, currentOwnerName, onConfirm, onCancel }: Props = $props();
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			console.error('[ClaimOwnershipModal] Submit failed:', err);
		} finally {
			isSubmitting = false;
		}
	}
	...
	function handleBackdropClick(e: MouseEvent) {
		// Close on backdrop click (less destructive than delete)
	...
	// Simple implementation without external library per D-071
	$effect(() => {
		if (modalElement) {
...
>
	...
	>
		...
		<div class="border-b border-neutral-200 px-6 py-4 dark:border-neutral-800">
			<h2 id="claim-modal-title" class="text-lg font-semibold text-neutral-900 dark:text-neutral-100">
				{t('devices.claim.modal.title')}
			</h2>
		</div>
		...
		<form onsubmit={handleSubmit} class="space-y-4 px-6 py-4">
			<!-- Info message -->
			<div class="flex gap-3 rounded-lg bg-primary-50 p-3 dark:bg-primary-950">
				<svg
				...
				<div class="text-sm text-primary-900 dark:text-primary-100">
					...
					<p class="mt-1">
						{#if currentOwnerName}
							{t('devices.claim.modal.body').replace('{owner}', currentOwnerName)}
						{:else}
							{t('devices.claim.modal.bodyUnowned')}
						{/if}
					</p>
				</div>
			</div>
